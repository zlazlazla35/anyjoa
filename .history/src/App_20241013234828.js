...
function App() {
  ...
  return (
    <div className="App" onClick={((e)=>{
      // console.log(userSearchOn.current.contains(e.target));
      // console.log(userSearchOn2.current.contains(e.target));
      if(searchInput && userSearchOn.current.contains(e.target) && userSearchOn2.current.contains(e.target)) {
        setSearchInput(true);
      }else{
        setSearchInput(false)
      }
      ...
      <Reset />
        <div className={`headerWrap ${scroll}`} >
              ...
              <div className='navRight'>
                {
                  !searchInput
                  ? <button className='searchIconBox' ref={userSearchOn} onClick={(()=>{ setSearchInput(true) })}>
                ...
                <div className= {searchInput ? 'searchInputBox on' : 'searchInputBox'} >
                  <FontAwesomeIcon className='searchIcon' icon={faMagnifyingGlass}/>
                  <input className='userSearch' ref={userSearchOn2} placeholder='제목 검색' />
                </div>
              </div>
          </div>
        <Routes>
