...
function AniDetail({setPopUp, clickAniInfo, cartCheck, setCartCheck}){
                ...
                <div className='aineMorePopup'>
                    ...
                    <div className='aniMoreContent'>
                        ...
                        <button className='add' onClick={(()=>{
                            ...
                            let isExisting = test.some(
                                (data) => data.id === clickAniInfo.id
                            );

                            if(isExisting){
                                setCartCheck(faPlus);
                                let infoIndex = test.findIndex((a)=>{
                                    return a.id === clickAniInfo.id
                                })
                                test.splice(infoIndex, 1);
                                localStorage.setItem('cart', JSON.stringify(test));

                                return;
                                }if(!isExisting){
                                    test.push(clickAniInfo);
                                    localStorage.setItem('cart', JSON.stringify(test));
